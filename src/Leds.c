...
{
    if (ref < 0)
        return 0xFF; // Erro se referência for inválida
...
void apply_led_and_buzzer_feedback(int ref, uint8_t diff, uint8_t status)
{
    ...
    if (diff == 1)
    {
        set_led_color(GREEN);                            // Alinhamento perfeito
        set_buzzer_level(BUZZER_A, WRAP_PWM_BUZZER / 5); // Tom mais alto
        if (status == 1)
        {
            printf("=== Realizando a fusão das fibras...\n");
        }
        else if(status == 0)
            printf("A fibra está alinhada perfeitamente!\n");
    }
    else if (diff == 2)
    {
        set_led_color(YELLOW); // Aproximado
        set_buzzer_level(BUZZER_A, WRAP_PWM_BUZZER / 50);
        printf("A fibra está alinhada, mas não próxima do ponto central.\n");
    }
    else
    {
        set_led_color(ORANGE); // Desalinhado
        set_buzzer_level(BUZZER_A, WRAP_PWM_BUZZER / 100);
        printf("A fibra está desalinhada!\n");
    }
}
